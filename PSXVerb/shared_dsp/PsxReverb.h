...
class PsxReverb {
public:
    PsxReverb() : work_buf_(nullptr), work_capacity_samples_(0), initialized_(false) {}
    ...
    void Init(float fs_io, const PsxPreset& preset) {
        ...
        // Allocate or grow shared buffer if preset needs more space
        if (!work_buf_ || work_size_samples > work_capacity_samples_)
        {
            delete[] work_buf_;
            work_buf_ = new int16_t[work_size_samples];
            work_capacity_samples_ = work_size_samples;
        }
    ...
    void SetDecayTime(float normDecay) {
        const float base = std::max(1e-5f, std::abs(preset_base_.vWALL_f));
        const float maxScale = std::clamp(0.99f / base, 0.5f, 10.0f);
        const float minScale = 0.5f;
        const float midScale = 1.0f;
        const float midNorm = 0.5f;
        float wall_scale = 1.0f;

        if (normDecay <= midNorm) {
            const float t = normDecay / midNorm; // 0..1
            wall_scale = minScale + t * (midScale - minScale);
        } else {
            const float t = (normDecay - midNorm) / (1.0f - midNorm); // 0..1
            wall_scale = midScale + t * (maxScale - midScale);
        }

        float target = preset_base_.vWALL_f * wall_scale;
        // Clamp to stable range to avoid runaway feedback
        p_.vWALL_f = std::clamp(target, -0.995f, 0.995f);
    }
...
private:
    ...
    WorkArea work_;
    int16_t* work_buf_;
    uint32_t work_capacity_samples_;
